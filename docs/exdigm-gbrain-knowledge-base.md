...
- `reference/exdigm-agent-api-capability-catalog`: Agent API를 route가 아니라 Exdigm 업무 capability catalog로 쓰는 기준
- `reference/exdigm-agent-api-surface-distillation`: Agent API route를 lookup/tool/workflow/internal/hidden surface로 정제하는 기준
- `reference/exdigm-hermes-agent-usage-guide`: Hermes container agent가 capability를 선택하고 gap을 보고하는 방법
- `reference/exdigm-work-completion-gate`: Exdigm 코드 작업 완료 전 code_map/GBrain/재사용 확인 게이트
- `project/exdigm-core-feature-pages`: Hermes Agent API, 데이터 추출, 파일 업로드, JD Processor, 프로젝트 workflow 상세 시작점
...
기계적 연결 manifest는 `manifests/exdigm-knowledge-map.json`에 둔다.

중요: raw code_map 결과를 그대로 GBrain에 복사하지 않는다. GBrain에는 에이전트가 작업을 시작할 때 정밀 타겟팅에 필요한 entrypoint, domain service, capability registry, 대표 테스트, reuse rule만 남긴다.
