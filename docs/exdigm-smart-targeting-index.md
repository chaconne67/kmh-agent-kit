...
| Agent API가 너무 많아 Hermes가 route를 헷갈린다 | `헤르메스 에이전트 API` | `reference/exdigm-agent-api-surface-distillation`, `reference/exdigm-agent-api-capability-catalog` | `manifests/exdigm-agent-api-surface.json`, `accounts/services/hermes_provisioning.py`, `accounts/agent_capabilities/registry.yaml` |
