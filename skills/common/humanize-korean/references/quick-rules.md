# Quick Rules — 단일 실행용 (v2.0)

`humanize-korean` 스킬이 한 번의 실행에서 탐지·윤문·자체검증을 끝내기 위해 사용하는 슬림 룰북. 본진 `ai-tell-taxonomy.md`에서 S1·S2 핵심 패턴과 처방을 추렸다.
...
위반 시: 해당 편집을 되돌리고 다시 윤문한 뒤 재점검한다. 자체 루프는 최대 1회만 수행한다. 문제가 남으면 `final.md`의 `HUMANIZE-SUMMARY` 블록에 미통과 항목 수를 적는다.

## 등급 기준 (자가 채점)

- **A**: S1 잔존 0, S2 잔존 2 이하, 변경률 10~25%, 자체검증 6항 모두 통과
- **B**: S1 잔존 0, S2 잔존 4 이하, 자체검증 5항 이상 통과
- **C**: S1 잔존 1~2 또는 자체검증 4항 이하 통과 — 원문 대조 검토 권고
