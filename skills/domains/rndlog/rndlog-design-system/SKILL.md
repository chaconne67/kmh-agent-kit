...
- 메인 화면 제목 위에 아이브로우(작은 분류·상위 메뉴 문구)를 넣지 않는다. 화면의 첫 제목은
  해당 메뉴명 하나로 표시해 제목 위계를 단순하게 유지한다.
- RNDLOG 화면을 기업자금 관리로 표현하지 않는다. 제목과 설명은 RNDLOG의 실제 업무를 따른다.
- 에이전트의 제작·수정·구현 설명을 제품 화면에 넣지 않는다. 무엇을 만들었는지, 어떤 기술로
  구현했는지, 무엇을 없애거나 유지했는지는 대화에서 보고한다. 화면에는 사용자의 업무 판단과
  조작에 필요한 정보, 입력 안내, 처리 결과와 오류만 표시한다.
...
- 제품의 데이터 표는 `static/css/input.css`에 정의된 `.darkhead-table`을 사용한다.
  머리글 색·글자·셀 여백·구분선은 공통 스타일을 재사용하고 페이지별 표 스타일로 대체하지 않는다.
  목록 내용이나 행 편집을 바꿀 때도 다크헤드 스타일을 유지한다. 좁은 화면에서는 표 영역 안에서
  가로 스크롤을 제공한다.
