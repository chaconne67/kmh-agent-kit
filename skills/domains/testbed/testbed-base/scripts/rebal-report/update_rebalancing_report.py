...
def update_trade_history(ws, trades):
    ...
    # 빈 행 정리 (베이스 템플릿의 잔여 빈 서식행 제거 → 거래가 헤더 바로 아래부터 연속되도록)
    empty_rows = [r for r in range(ws.max_row, 1, -1)
                  if all(ws.cell(r, c).value in (None, '') for c in range(1, ws.max_column + 1))]
    for r in empty_rows:
        ws.delete_rows(r)
    if empty_rows:
        print(f'  빈 행 {len(empty_rows)}개 정리')
...
def update_port_change(ws, rday_str, reason_text='정기리밸런싱'):
    ...
    if date_col is None:
        # 폴백: 날짜가 수식(예: ='MP내역'!A13)인 경우 → R5 헤더의 'MP생성일자' 컬럼 사용
        header_r5 = build_header_map(ws, 5)
        for header, col in header_r5.items():
            if 'MP생성일자' in header:
                date_col = col
                break
    if date_col is None:
...
def main():
    ...
    parser.add_argument('--work-dir', required=True, help='JSON이 있는 로컬 임시 작업 디렉토리')
    parser.add_argument('--source-xlsx', required=True, help='Drive에서 받은 로컬 21번 원본')
    parser.add_argument('--output-xlsx', required=True, help='새 21번 xlsx 출력 경로')
    ...
    if not work_dir.is_dir():
        print(f'[ERROR] 작업 디렉토리 없음: {work_dir}')
        sys.exit(1)
    if not source_path.is_file():
        print(f'[ERROR] 로컬 원본 xlsx 없음: {source_path}')
        sys.exit(1)
    if source_path == output_path:
        print('[ERROR] 원본과 출력 경로가 같습니다.')
        sys.exit(1)
    if output_path.suffix.lower() != '.xlsx':
        print('[ERROR] 출력 파일은 .xlsx여야 합니다.')
        sys.exit(1)
    if not output_path.parent.is_dir():
        print(f'[ERROR] 출력 디렉토리 없음: {output_path.parent}')
        sys.exit(1)
    if output_path.exists() and not args.dry_run:
        print(f'[ERROR] 출력 파일이 이미 있습니다: {output_path}')
        sys.exit(1)

    # 1. JSON 로드
    json_path = get_json_path(work_dir, args.schedule_id)
