...
추출 코드·프롬프트·규칙을 수정하기 전에 다음 잠금을 작성한다.

1. **일반화 근거**: 단일 사례의 값·표현·배치가 아니라 반복 가능한 구조·계약을 수정함을 입증한다.
2. **회귀 대조**: 실패 사례와 원인이 다른 기존 통과 사례를 실제 운영 처리 경로로 수정 전후 대조한다.
3. **판정**: 1과 2를 입증하지 못하거나 수정 대상이 아닌 출력 변화가 검증되지 않으면 수정하지 않는다.
