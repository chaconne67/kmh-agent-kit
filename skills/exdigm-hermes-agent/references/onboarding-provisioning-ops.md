...
| 에이전트API key | Django 암호화 DB + 직원 profile `.env` | Django, 로컬 Exdigm MCP |
...
- 개발 환경에서 URL과 read token이 모두 있을 때만 연결한다.
- 운영 profile에는 연결하지 않는다.
- 개발 GBrain 장애는 Exdigm MCP·에이전트API·Telegram 작동을 막지 않는다.

## Deployment

- Django 일반 배포와 Hermes 배포는 별도 명령이다.
- 개발: `scripts/deploy/deploy_hermes.sh dev <git-commit>`.
...
- 삭제: Django 사용자 삭제 + FK 없는 offboard 요청 확정 → 배달 작업자 →
  프로비저닝 `offboard` → container 제거 → profile `.env` 제거.
