...
| --- | --- | --- | --- |
| `document` | yes | 담금질할 문서 경로 | none |
| `topic` | yes | slug 식별자. 센티널 마커에 사용 | infer or ask |
| `forge_dir` | no | 산출물 디렉토리 | `docs/forge/codex/{topic}/` |
| `context` | no | 참조 문서 목록 | none |

Derived:
- `stem`: document filename without extension
- source document: original `document`
- rulings: `{forge_dir}/debate/{stem}-rulings.md`
- sentinel: `<!-- forge:{topic}:{stem}:complete:{ISO 8601 timestamp} -->`
...
concrete rebuttal evidence. Apply accepted source-document changes before the
next critic round.
...
Copy the source document to `{forge_dir}/debate/{stem}.md` before tempering if it
is not already there. The copy is debate evidence only; the final accepted
changes are applied to the original `document`.
...
1. **Precheck**
   - Confirm git repo and document existence.
   - Confirm subagent tool availability.
   - Select author, critic A, and critic B roles.
   - If the original `document` already ends with the sentinel, stop as complete.
...
5. **Author Rebuttal And Revision**
   ...
   - Evidence priority: `code_reference` > `execution_result` > `data` >
     `logical_reasoning`.
   - Apply the author's accepted source-document changes.
...
8. **Finalize**
   - Apply accepted changes directly to the original `document`.
   - Append sentinel marker to the original `document` as the last line.
...
- [ ] Accepted author changes applied to the source document.
- [ ] Rulings and debate log updated.
- [ ] Original `document` updated with accepted changes and sentinel marker.
