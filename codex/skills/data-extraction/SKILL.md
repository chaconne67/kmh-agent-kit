...
uv run python -m tools.code_map.impact --topic "후보자 등록"
uv run python -m tools.code_map.impact --topic "이력서 최신 버전"
uv run python -m tools.code_map.impact --file data_extraction/services/save.py
```

새 command, route, upload path, 저장 규칙, 테스트가 추가되면 `tools/code_map/semantic_topics.toml`도 함께 갱신하고 아래를 실행한다.
...
수정 후에는 코드 지도와 focused 테스트를 같이 확인한다.
