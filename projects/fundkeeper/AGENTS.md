...
- 작업 위치: `C:\Users\chaconne\projects\fundkeeper`. 코드·Git·검증·배포는 아래 원격 서버의 기존 경로를 사용합니다.
- 새 세션은 대화 이식 없이 `~/.gbrain-agent.md`의 현재 래퍼로 공용 `project/fundkeeper-operating-context`를 먼저 읽습니다.
- GBrain은 전역 컨트롤타워 카드의 공용 조회 경로로 `project/fundkeeper-operating-context`를 읽습니다. GBrain 본체는 DB 서버에 유지합니다.
