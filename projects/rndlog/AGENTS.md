...
- 고객사 원천자료, 회사별 리서치, 보고서 작업본과 PDF 산출물은 이 폴더에서만 관리합니다.
...
- `inputs/`는 카카오톡으로 받은 원천자료, `research/`는 회사별 조사자료,
  `reports/source/`는 보고서 작업본, `reports/output/`은 PDF 산출물입니다.
...
3. `rndlog` 스킬을 읽고 회사 자료 취합, 리서치, 보고서 작성, PDF 생성을 순서대로 수행합니다.
4. 조사 결과는 `research/`, HTML은 `reports/source/`, PDF는 `reports/output/`에 저장합니다.
...
- 고객사 PDF는 메인서버에서 `rndlog` 스킬의 WeasyPrint 경로로 생성하고 누락 파일이 없는지 확인합니다.
