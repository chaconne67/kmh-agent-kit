...
| 운영 DB | 중앙 서버 `CentralDB_postgres`의 `company_main` DB·`rndlog` 스키마, SSH 터널 `127.0.0.1:15432` |
...
- 확인되지 않은 회사 고유정보·연구원·논문·기관명을 만들지 않습니다. 작성 시나리오의 수치와 결과는
  `실측 아님`과 회사 확인 필요 여부를 표시합니다.
...
- 자료의 충분성이나 제출 가능성을 대신 판단하지 않습니다. 회사 제공 자료와 검증 가능한 리서치를
  바탕으로 요청 문서를 작성하며, 부족한 활동 내역은 연구 시나리오로 구체화하고 최종 검토와 사용은
  회사가 결정합니다.
